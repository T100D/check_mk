Local checks created for monitoring information from Linux and Windows hosts with check_mk
  ...
  - check-status.cloud.microsoft.sh - Script to read the contents of the Microsoft status webpage and store locally
  - mk_check-status.cloud.microsoft - Checkt to read the contents of the above file and present it locally
  - Script to read the export and returns the build number of a programm called aramis
  - check_fileage.ps1 - (windows) - Check last write timestamp on a file. Problem is that MS does not regular updates the timestamp :-( 
  - cleansimple.sh - Our cleanup script for Docuwiki
